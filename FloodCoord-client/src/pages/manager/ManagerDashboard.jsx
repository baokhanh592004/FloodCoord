...
// Import thêm importApi mà bạn đã tạo
...
import {
  ...
  DocumentArrowUpIcon, // Icon mới cho Import
  DocumentArrowDownIcon, // Icon mới cho Template
} from '@heroicons/react/24/outline';
import {
  BarChart, Bar, PieChart, Pie, Cell,
  XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer,
} from 'recharts';
import { toast } from 'react-hot-toast'; // Giả định bạn dùng react-hot-toast để thông báo
...
export default function ManagerDashboard() {
  ...
  const handleImport = async (e, type) => {
    ...
    try {
      if (type === 'SUPPLY') {
        ...
        toast.success("Import phương tiện thành công!");
      }
      loadData(); // Tải lại dữ liệu sau khi import thành công
    } catch (error) {
      toast.error(error.response?.data || "Import thất bại, vui lòng kiểm tra lại file!");
    } finally {
      setImporting(false);
  ...
  const handleDownloadTemplate = async (type) => {
    try {
      ...
      let fileName = "";
      if (type === 'SUPPLY') {
        response = await importApi.supply.getTemplate();
        fileName = "Mau_Import_Vat_Tu.xlsx";
      }
  ...
  // CẬP NHẬT LOGIC ĐỘI CỨU HỘ: Tính toán đủ 4 trạng thái
  const teamStats = useMemo(() => ({
    total: teams.length,
    available: teams.filter(t => t.status === 'AVAILABLE').length,
    busy: teams.filter(t => t.status === 'BUSY').length,
    offDuty: teams.filter(t => t.status === 'OFF_DUTY').length,
  }), [teams]);
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full">
      ...
      {/* Header Updated with Import Buttons */}
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tổng quan Quản lý</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <input 
              ...
              className="hidden" 
              accept=".xlsx, .xls"
              onChange={(e) => handleImport(e, 'SUPPLY')} 
            />
            <button
          ...
          {/* Nút Tải Template Vật Tư */}
          <button
            onClick={() => handleDownloadTemplate('SUPPLY')}
            className="p-2 text-gray-500 hover:bg-gray-100 rounded-md border border-gray-200"
          ...
          <button
            onClick={loadData}
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 disabled:opacity-60 transition-all shadow-sm"
          >
      ...
      {/* Thông báo đang xử lý file */}
      {importing && (
        <div className="bg-blue-50 border-l-4 border-blue-400 p-4 animate-pulse">
           <p className="text-sm text-blue-700 font-medium">Hệ thống đang xử lý file Excel, vui lòng đợi trong giây lát...</p>
        </div>
      )}
      ...
      {/* ===== STAT CARDS: Đội cứu hộ - Đã cập nhật 4 trạng thái ===== */}
      <div>
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard icon={<CheckCircleIcon className="h-6 w-6" />} count={teamStats.available} label="Đội sẵn sàng" color="green" />
          <StatCard icon={<ArrowRightIcon className="h-6 w-6" />} count={teamStats.busy} label="Đang làm nhiệm vụ" color="yellow" />
          <StatCard icon={<ExclamationTriangleIcon className="h-6 w-6" />} count={teamStats.offDuty} label="Đội đang nghỉ" color="red" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard icon={<ExclamationTriangleIcon className="h-6 w-6" />} count={supplyStats.expiringSoon} label="Sắp hết hạn" color="yellow" />
          <StatCard icon={<ExclamationTriangleIcon className="h-6 w-6" />} count={supplyStats.expired} label="Đã hết hạn" color="red" />
          <StatCard icon={<CheckCircleIcon className="h-6 w-6" />} count={supplyStats.total - supplyStats.expiringSoon - supplyStats.expired} label="Lô hàng an toàn" color="green" />
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <ActionCard
            ...
            badge={`${teamStats.available} sẵn sàng`}
            badgeColor="teal"
